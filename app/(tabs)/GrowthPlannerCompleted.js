  ...
  const getSavedGrowthPlanner = async () => {
    try {
      const response = await authFetch.get(
        `/learning/get-saved-growth-responsesid/${item._id}`
      );
  ...
  useEffect(() => {
    if (savedPlan) {
      setGrowthData({
        ...
        percentageCompleted: savedPlan.percentageCompleted || 0,
        status: savedPlan.status || "Not Started",
      });
    }
  }, [savedPlan]);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      {growthData?.tasks?.length > 0 ? (
        <View>
          ...
          <View>
            <Text style={styles.completionText}>This planner is submitted</Text>
            ...
            {completionPercentage > 100 ? (
              ...
              <Text style={styles.completionText}>
                To make changes to this planner, please contact your church
                administrator.
              </Text>
            )}
          </View>
...
GrowthPlannerCompleted.defaultProps = {
  item: {
    description: "",
    title: "",
    forms: [],
...
export default GrowthPlannerCompleted;
